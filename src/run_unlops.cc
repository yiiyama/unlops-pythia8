...
main(int argc, char* argv[])
{
  ...
  while (iEvent++ != nEvents) {
    ...
    // Pick a channel (process) to generate. If nPartons != 0, set the event as a counter term with a 50% chance.
    ...
    // If we are at the end of the LHE file for the selected process, we'll stop generating events altogether.

    if (atEnd)
      break;

    // Fill the TTree branches with the event information.
