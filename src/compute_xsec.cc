...
main(int argc, char* argv[])
{
  ...
  if (std::abs(pythia.info.lhaStrategy()) == 4) {
    meanW = sumW / pythia.info.nSelected();
    meanAbsW = sumAbsW / pythia.info.nSelected();
  }
  else {
    meanW = pythia.info.sigmaGen() * 1.e+9;
    meanAbsW = sumAbsW / sumW * pythia.info.sigmaGen() * 1.e+9;
  }

  double wnorm(lheInput->sumAbsW() / pythia.info.nSelected());
  
  std::cout << "(Channel meanW) (Channel mean|W|) (Channel wnorm) (Channel sigmaGen)" << std::endl;
  std::cout.precision(10);
  std::cout << meanW << " " << meanAbsW << " " << wnorm << " " << (pythia.info.sigmaGen() * 1.e+9) << std::endl;
